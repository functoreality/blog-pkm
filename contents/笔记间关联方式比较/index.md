...
* <input type="checkbox" name="fchk" id="fold66"><label for="fold66">**标签**：用标签可以体现多分类，也能建立基于主题与话题的集体关联，但仍有信息不完善等局限性。标签所起到的作用原则上也都可被 MOC 替代。</label>
	...
	* <input type="checkbox" name="fchk" id="fold92"><label for="fold92">用标签来查找特定笔记的时候，有观点认为其易用性可能不如文件夹和 MOC。这是因为比起逻辑关系，人脑更擅长记忆空间位置关系，而依据标签的查找本身不涉及空间感知，主要是依据笔记的逻辑关系进行的。</label>
		...
		* 在使用文件夹查找相应笔记时，需要依次点开一个个文件夹，而每个中间文件夹所处的空间位置（在多数情况下）大体固定。若使用 MOC 查找，则需要沿着链接不断跳转到下一层 MOC，每条链接在特定 MOC 中的位置往往还是自己主动设定的。这两种笔记组织方式能提供较强的空间位置感。
