...
* 各位读者朋友，好久不见！
* 三年前，我连续写了几篇关于笔记系统的文章，介绍了一些对我来说很好用，并且我现在也一直在用的做法。
...
* **技术特性**：如果我们往下看一层，会发现每个工具都包含了许多的技术特性。
	...
	* 了解「工具是技术特性的组合」这一点，还能帮助我们更好地理解两件事情：工具选择，和多种工具的联动。
		* 理解「工具选择」：选择工具的核心，其实是技术特性需求的匹配——什么工具提供的特性组合对自己最有价值。
			* 这里需要强调的是**个体差异**：别人看重的技术特性，未必也对你重要。
				* 比如，如果某个工具的卖点是多人实时协作，但你关心的只是自己一个人的笔记，那这个特性对你就没有价值。
				* 所以在工具选择上，没必要盲目跟风。
			* 划清自己的需求边界，然后在边界内探索和寻找最优解，这才是理性的选择策略。
			* 当然，这里说的「需求边界」也不仅仅是指眼下，将来的潜在变化有时也要考虑在内。
		...
		* 理解「工具联动」：如果你需要的多种技术特性，没有一个工具能同时具备，这时候就没必要一味追求 all-in-one，可以考虑组合使用多种工具。
	...
	* 对于提供价值类的技术特性，有一个重要的提醒：更多关注对自己最有用的那些特性，而不是追求「越多越好」。
		...
		* 如果是刚接触和上手一个新工具，也不必一上来就研究它的全部功能。
			* 先想清楚自己要用它来做什么，然后只关注和学习自己所需的「最小技能集」。
			* 在使用逐步深入了以后，可以再进一步了解、探索和扩展。
* **笔记方法**：如果往下再看一层，我们会发现，一个技术特性之所以对我们有价值，通常是因为希望能够用某种特定的方式来使用笔记。换句话说，我们打算采用特定的笔记方法。
	* 比如说，许多笔记软件都能罗列出所有打了某个标签的笔记。
		* 这个技术特性之所以对我们有用，是因为我们打算提取某条笔记时，可能会搜索之前为这个笔记打过的标签——这就是一种特定的笔记方法。
		* 当然，这个方法能够使用的前提，是我们添加笔记时已经为它打上了标签。
	...
	* 现在我们把关注点聚焦到「笔记方法」本身。市面上关于笔记方法的讨论非常多。
	...
	* 不过，我想分享一个对我自己比较好用的思考方式：「方法单元」。
		...
		* 大家平时看到过的那些有名有姓的「方法」，其实往往是许多这样的方法单元的组合。
			* 比如，《卡片笔记写作法》这本书里，我们就可以找出下面这些方法单元（只列举很小一部分）：
				* **记录闪念笔记**：当你在散步、交谈或任何场合突然产生想法时，马上用餐巾纸或随身携带的小笔记本记录下来，作为提醒。
					* 这里的场景是「灵感突发」，做法是「立即简要记录」。
				* **及时处理闪念笔记**：记录闪念笔记后，尽快（一般 1–2 天内）把它改写成更完善的形式。
				...
				* **管理区分生命周期**：将笔记分为闪念笔记（只用一两天）、项目笔记（几周到几年后停用，以项目完成为界）、永久笔记（一直保留，即使内容过时）。
...
* **笔记功能**：如果我们再往下深入一层，会发现每个笔记方法之所以对我们有用，是因为它能起到某种实际的功能。
	...
	* 如果说即时性功能主要关注的是「现在」，那么长期性功能关注的就是「未来」——如何让笔记在创建之后的很长一段时间里，能够为我们带来持续的价值。
		* 这里有三种主要的具体功能。
		* 首先是**使用辅助**。我们对笔记做出特定的处理，实际上是要为未来的自己铺路。
...
* **笔记内容**：我们已经提到，笔记可以看成是内容的载体，它所承担的各种功能是为了内容服务的。
	...
	* 外源类内容：这部分内容不是由你本人创造，而是从外部获取的。
		...
		* **第三种是临时类内容**，只能依靠当下记录。
			*  比如会议记录，与人沟通时的要点，听课或访谈时的关键信息，或者是做某件事过程中的历程记录。
				* 这些内容有一个共同特点：错过了就没有了。
			* 还有一些挺有意思的例子。比如，你可能会偶然从身边人的做法中学到一些实用的技巧——
				* 朋友表达感谢、歉意或祝福时的巧思，
				* 别人学术报告里独特的开场设计，
				* 或是街边广告上看到的别出心裁的文案。
	...
	* 内源类（具体内容）：说完了外源，我们再来看看由内而生的内容。
		* 内源类内容中的第一种，我们可以把它叫做**外源衍生**。
	...
	* 到这里，我们从内容的来源和性质出发，梳理了不同类型的内容需要哪些不同的功能。
		...
		* 当然，实际使用中，这些类别并不是泾渭分明的。
			* 同一个笔记系统里，可能同时存在多种类型的内容；
			* 同一条笔记，也可能随着你的加工而逐渐从外源转化为内源。
			* 这都是很正常的现象。
	* 内容质量：在这个小节的最后，再给读者朋友们留下一个小小的提醒吧。
...
* **使用场景**：现在，我们终于来到了这个框架的最底层。
	...
	* 场景驱动的价值
		...
		* 注：需要解释一下，这里的「使用」也不一定要按狭义的方式理解呀。
...
* 尾声：怎么用好这个框架
	...
	* 向下分析：如何面对新工具和新方法
		...
		* 正是因为如此，你不必每次看到新工具、新方法都陷入 FOMO（fear of missing out，担心错过）的焦虑心态。
			* 要清楚，不同人的需求和目的不同，场景和条件也不同。
			* 对别人很有用的工具方法，未必也对你有用；
			* 同一个工具方法中别人最看重的方面，可能和对你最有用的方面也并不相同。
		* 想清楚自己需要什么，新的工具和方法能够提供什么，
			* 是不是帮助自己发现了之前没意识到的新场景、新内容、新功能、新的方法单元，
			* 然后再针对性地尝试、选用。
		* 当然，你也可以**积极尝试**，先用上再说，合则用、不合则弃。
			...
			* 这篇文章的框架，某种程度上就像是一个事后的评价标准。
				* 它可以用来回顾那些方法是否真的有用，以及为什么有用。
				* 有些时候，它或许还能启发你，帮你发现自己之前没意识到的需求。
			* 如果确实对自己有用，尽管采纳就好。
		* 如果目前对自己没用，但它确实比较新颖、或许能有参考价值，也不妨把这些内容留存下来。
...
* 欢迎在评论区分享你的想法和经验！
	...
	* **话题二：漏洞征集**。这篇文章里讲的内容，你有没有发现什么可能的逻辑漏洞？
		...
		* 相信读者朋友们能够理解，我不那么想追逐热点，还是希望能给大家带来一个许多年稳定、能够指导实践的体系。
