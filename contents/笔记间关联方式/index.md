...
* <input type="checkbox" name="fchk" id="fold4"><label for="fold4">姑且对「联系」与「关联」两个词做这样的区分：不同内容之间的「联系」是客观存在的，需要人来发现；「关联」则是笔记的一部分，它由人主动建立起来，用以表达内容间的联系。</label>
...
* <input type="checkbox" name="fchk" id="fold20"><label for="fold20">这种丰富的关联还能帮助发现新的联系、形成新的想法，使笔记系统起到卢曼卡片盒的「惊喜发生器」的效果。</label>
...
* <input type="checkbox" name="fchk" id="fold141"><label for="fold141">**空间邻近**：使用电子笔记时，笔记间空间邻近关系的作用容易被忽略，不过它仍是一种有价值的关联。</label>
	...
	* <input type="checkbox" name="fchk" id="fold151"><label for="fold151">在某种意义上，卢曼所用的笔记序列可以看成大纲笔记在纸质时代的物理实现。</label>
...
* <input type="checkbox" name="fchk" id="fold189"><label for="fold189">**链接**：最明确、精准的成对关联，也是卡片盒笔记法的要点之一。建立链接时通常需要附带一些文字说明，以使该关联更加具体明确。</label>
	* 链接在维基、学术引文网络等场合也十分常见。不过卡片盒笔记法中所用链接的含义与此并不完全相同，其中重要的一点是要与笔记原子化原则配合，从而允许建立更细粒度的、更精准的链接。
	* <input type="checkbox" name="fchk" id="fold191"><label for="fold191">有观点基于「思维的网状结构」而主张使用大量链接来组织笔记。这类笔记的阅读体验确实会与传统线性文章有所不同，可通过查看相应的公开笔记来获得一些直观感受，例如 Andy Matuschak 的系列文章。</label>
	...
	* <input type="checkbox" name="fchk" id="fold193"><label for="fold193">就卢曼本人而言，他将二代卡片盒作为自己在学术前沿探索、思考、创新的工具，并使链接有了更丰富的含义，其重要性甚至可以超过笔记的内容本身。这是在（通常意义上的）知识管理中几乎不可能出现的情况。</label>
	...
	* <input type="checkbox" name="fchk" id="fold203"><label for="fold203">建立链接时，附上一些文字描述通常会有帮助：这能帮助展现关联的背景，使其含义更丰富；简明扼要的文字也能在日后回顾笔记时提供引导。</label>
		* <input type="checkbox" name="fchk" id="fold204"><label for="fold204">Andy Matuschak 的笔记中有一篇是《优先使用经过标注的关联》（[中文翻译](https://zhuanlan.zhihu.com/p/425763923)，[英文原文](https://notes.andymatuschak.org/z7pGUpz2fQsHHUPbjThz85xXPvHwrmikAeYH4)），其中强调了对链接进行标注的重要性。</label>
...
* <input type="checkbox" name="fchk" id="fold211"><label for="fold211">**反向链接**：双链笔记软件的重要贡献。它通常是与链接配合的另一面，属于成对关联，而在某些情况下也能用来构建集体关联。</label>
	* 反向链接的展示利用了双链笔记软件提供的反向搜索功能。我们在笔记 A 中建立指向笔记 B 的链接之后，可以立刻在笔记 B 的下方看到笔记 A 引用了它，而无需进行额外的操作。
	* <input type="checkbox" name="fchk" id="fold213"><label for="fold213">如果只考虑追溯引用的目的，这实际上在非双链笔记中也能实现，只需要每次建立 A 到 B 的链接时，再同时在 B 中建立指向 A 的链接即可。双链笔记软件则给了用户省下这一步操作的可能性。</label>
		* 卢曼在[《与卡片盒交流》](https://mp.weixin.qq.com/s/_aPS2ol1DxiCFX-xfl-8yg)一文中就提到了这样的做法，在建立链接时，「也要马上在被链接到的笔记中记录下反向链接。」
	* <input type="checkbox" name="fchk" id="fold215"><label for="fold215">不过同样作为成对关联方式，反向链接仍与直接链接不同。直接链接是主动建立的关联，其建立可以有选择性，也能补充相应的文字描述；而被动建立的反向链接不具有这些特点，其实用性比起直接链接要相对低一些。</label>
	...
	* <input type="checkbox" name="fchk" id="fold221"><label for="fold221">反向链接还为笔记软件提供了一种新的用法。我们可以建立指向一条空笔记的链接，在这样的链接较多之后，从这条空笔记的反链列表里就可以看到许多与之相关的内容。这可认为是依靠反链建立了某种集体关联。</label>
		* <input type="checkbox" name="fchk" id="fold222"><label for="fold222">Roam Research、Logseq 等笔记软件中的标签是正文中形如 `#标签` 的文本，而这些软件实际上将标签当成了一种特殊的链接。我们要搜索这个标签下的内容时，其实是点进这个标签所对应的（空）笔记，并查看它的反链列表。也就是说，其中的标签搜索功能实际上是通过反链搜索来进行的。</label>
	...
	* <input type="checkbox" name="fchk" id="fold226"><label for="fold226">此外，反链在 daily notes 工作流中也可发挥其作用。这允许我们将一些临时产生的信息先暂时存放在 daily notes 中，而只在将来需要的时候才将之转移进相应主题的笔记。</label>
...
* <input type="checkbox" name="fchk" id="fold231"><label for="fold231">**MOC（Map of Contents，内容地图）**：基于链接建立起的关联，也是最强大、最灵活的集体关联方式。MOC 的引入也使笔记系统具有了更丰富的结构。</label>
	...
	* <input type="checkbox" name="fchk" id="fold235"><label for="fold235">MOC 的引入使笔记系统中链接的类型变得更为多样。原来只有普通笔记之间的链接，而现在多了 MOC 指向普通笔记的链接，此外还可以有普通笔记指向 MOC 的链接。</label>
	...
	* <input type="checkbox" name="fchk" id="fold239"><label for="fold239">此外，如果一个 MOC 中引用的笔记数量变得比较多了，我们还可以把它拆成几个小的 MOC，然后在一条新笔记中存放指向这些小 MOC 的链接。</label>
	...
	* <input type="checkbox" name="fchk" id="fold242"><label for="fold242">在用于进行主题汇总时，MOC 将显得更为强大。它使我们能够更有效地管理这一主题，加深对其中内容的理解。</label>
		...
		* <input type="checkbox" name="fchk" id="fold246"><label for="fold246">在进一步引入指向 MOC 的「高阶 MOC」之后，其处理多主题的潜能将得到进一步的释放。</label>
	...
	* <input type="checkbox" name="fchk" id="fold258"><label for="fold258">在有了 MOC 之后，向笔记系统添加新笔记的工作流也有了更多内容，这包括将它加入相关的 MOC，以及建立一个新 MOC 等。</label>
