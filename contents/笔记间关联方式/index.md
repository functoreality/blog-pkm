...
* <input type="checkbox" name="fchk" id="fold4"><label for="fold4">也有人会从笔记的「结构」或者「组织方式」的角度进行类似的讨论，而「关联方式」或许提供了一种更好的切入点。</label>
	* <input type="checkbox" name="fchk" id="fold5"><label for="fold5">「笔记应该有结构，但也不能太有结构」的说法收获了不少人的赞同。从这个角度来看，笔记管理的一个重点便是确定其中的平衡位置，即回答「笔记应该有何种程度的结构」这个问题。而事实上，出现这种模棱两可的表述很可能已经意味着「结构」并不是最合适的评判标准了。</label>
	...
	* <input type="checkbox" name="fchk" id="fold8"><label for="fold8">使用「组织方式」一词也可能造成某些误解，仿佛以下所说的文件夹、标签、链接对应着完全不同类型的笔记系统，它们之间是非此即彼的关系。</label>
		* 实际上，它们是可以在笔记系统中同时存在的，只不过是用不同的方式描述了笔记之间的联系。「关联方式」的说法则能够给出这样的暗示。
	* <input type="checkbox" name="fchk" id="fold10"><label for="fold10">另外，强调「结构」或者「组织方式」还可能使我们出现更多关注形式而非内容的倾向。</label>
...
* <input type="checkbox" name="fchk" id="fold15"><label for="fold15">姑且对「联系」与「关联」两个词做这样的区分：不同内容之间的「联系」是客观存在的，需要人来发现；「关联」则是笔记的一部分，它由人主动建立起来，用来体现内容间的联系。</label>
...
* <input type="checkbox" name="fchk" id="fold21"><label for="fold21">关联可以大致分为成对关联和集体关联两种。前者如链接，一次只关联起两条笔记；后者如文件夹和标签，可以将许多笔记关联起来。分类和主题是建立集体关联的常用方式。</label>
	* <input type="checkbox" name="fchk" id="fold22"><label for="fold22">在分类系统中，如果许多条笔记被归入相同的类别，那么它们就基于这种「同类」的关系而被关联了起来。</label>
		* 一个类别还可以有许多子类。若其中一些笔记进一步被归到同一个子类之下，则它们之间就建立了相对来说更小范围的、更明确的、也更强的关联。
	* 笔记也可以从属于某个主题，或者与某个话题相关。这表达的关联也是类似的。
* <input type="checkbox" name="fchk" id="fold25"><label for="fold25">在笔记间建立丰富的关联，能够帮助检索，使已有笔记的查找更加便捷、高效。</label>
...
* <input type="checkbox" name="fchk" id="fold29"><label for="fold29">这种丰富的关联还能帮助发现新的联系、形成新的想法，使笔记系统起到卢曼卡片盒的「惊喜生成器」的效果。</label>
...
## 常用的关联方式
* <input type="checkbox" name="fchk" id="fold38"><label for="fold38">**文件夹**：若要建立分类式的集体关联，使用文件夹是最简单、直观的方式，不过它不适合处理一条笔记有多个分类的情况。文件夹的功能理论上可被 MOC 替代。</label>
	* 文件夹是传统层级式分类法的代表。存放在同一个文件夹下的笔记有着相同的类别，它们通过这种方式被简单地关联了起来。
	* <input type="checkbox" name="fchk" id="fold40"><label for="fold40">文件夹分类管理方式的有效性相当程度上依赖于 MECE 原则，而实践中频繁出现的多分类情况往往造成严重的干扰。新加入的笔记可能同时属于多个类别；此外分类标准可能并不唯一，采用不同的标准可以给出不同的分类结果。</label>
		...
		* <input type="checkbox" name="fchk" id="fold44"><label for="fold44">至于「分类标准不唯一」，我们知道文学作品不只可以按地域分类，还可以按体裁分为「诗歌」、「戏剧」、「小说」、「散文」，按时代则是「古代」、「近代」、「现代」。</label>
		...
		* <input type="checkbox" name="fchk" id="fold48"><label for="fold48">为了应对这种情况，可以考虑使用系统的快捷方式、使用标签或者 MOC 来作为文件夹的替代品。</label>
			* 系统快捷方式允许从不同的文件夹下访问同一个文件。不过这不便于备份以及多设备间同步，在笔记文件位置改变（比如重新分类、挪入新建的子文件夹）时会失效，比较受限。
			* <input type="checkbox" name="fchk" id="fold50"><label for="fold50">一些笔记软件可以体现标签之间的层级关系，也就是所谓的「嵌套标签」。还用文学作品的例子，此时关于某个作品的一条记录可以同时有 `#文学作品/按地区分类/英美`，`#文学作品/按体裁分类/诗歌/十四行诗`这两个标签。</label>
	...
	* <input type="checkbox" name="fchk" id="fold55"><label for="fold55">而若 MECE 原则没能完美贯彻，或者查找笔记时没能（完整地）回忆起当时分类所依据的原则，我们提取已有笔记的过程将可能受到很大的干扰。</label>
		*  yuchen_lea 在 [请不要神化双链笔记](https://sspai.com/post/65273) 一文中作了这样的描述：
			> 如果一开始在错误的目录中寻找文件，可能需要浏览多个层级之后，才能意识到自己找错了地方，费时费力。而最坏的情况是：文件实际存在，但因为没有找到认为它不存在。
	* <input type="checkbox" name="fchk" id="fold58"><label for="fold58">考虑到个人认知有一个发展的过程，我们完全有可能在未来的某一天发现，根据自己之前的粗浅理解所设立的分类方式已经不再适用了。对文件夹管理方式来说，这种重新分类的过程可能是相当痛苦的。</label>
		* <input type="checkbox" name="fchk" id="fold59"><label for="fold59">使用标签进行的重分类则要相对轻松一些。使用标签的分类体系不像文件夹那样有排他性，这允许我们在不破坏旧的分类体系的情况下，为笔记补上新分类标准下的对应标签。</label>
		...
		* <input type="checkbox" name="fchk" id="fold62"><label for="fold62">使用 MOC 进行的重分类同样十分灵活，我们只需要建立一个新的 MOC（或在原有 MOC 的前面或后面开辟一段新的空间），用来存放新标准下的分类结果就可以了。</label>
			...
			* <input type="checkbox" name="fchk" id="fold65"><label for="fold65">即使在新标准的 MOC 建立完成以后，我们也没必要删除原来旧标准下的 MOC。只需要在旧版本中做适当的标注，并给出指向新版本 MOC 的链接即可。</label>
	...
	* <input type="checkbox" name="fchk" id="fold69"><label for="fold69">此外，文件夹下的内容以简单并列的方式呈现，但它们之中其实有一些存在更紧密的联系（其数量又不值得单独建立一个子文件夹），或者其关系值得进行特殊的说明。这类具体而精细的关联难以使用「同分类」的简单方式来表达。</label>
		* <input type="checkbox" name="fchk" id="fold70"><label for="fold70">若设定文件名以使一些文件显示在相邻位置，可以部分地表达前一种关联。</label>
			* 用文件名来管理排序是实现下方所说的「空间邻近」关系的方式之一。但这稍显麻烦，也没能在简单相邻之外提供更多信息。
			* 这样的邻近关系只有在文件依据名称进行排序时才能看到，且需要小心设定文件名，添加新文件时还需要避免它误插入本应相邻的文件中间。
			* <input type="checkbox" name="fchk" id="fold73"><label for="fold73">还可能遇到这样的情况：A 与 B 两篇笔记本不相关，安排在相隔较远的位置，但新添加的笔记 C 与二者都相关。</label>
	...
	* <input type="checkbox" name="fchk" id="fold79"><label for="fold79">原则上，文件夹所能实现的功能用标签、MOC 也都能实现，而其中 MOC 更为灵活。对于能够灵活使用 MOC 的人来说，文件夹的结构在笔记系统中并不是必要的。</label>
...
* <input type="checkbox" name="fchk" id="fold84"><label for="fold84">**标签**：用标签可以体现多分类，也能建立基于主题与话题的集体关联，但仍有信息不完善等局限性。标签所起到的作用原则上也都可被 MOC 替代。</label>
	* <input type="checkbox" name="fchk" id="fold85"><label for="fold85">相较文件夹给出的层级单分类方式，用标签能更加方便地处理多分类的情况。涉及特定主题、话题的内容也更适合通过标签来聚合，因为与一条笔记相关的主题、话题也往往是不唯一的。</label>
	...
	* <input type="checkbox" name="fchk" id="fold90"><label for="fold90">在早期阶段的笔记系统中，标签数量相对较少，所有标签都以平等的方式组织，标签和标签之间没有差别。但随着标签数量增多，这种平行的标签管理方式就显得不再方便。</label>
	...
	* <input type="checkbox" name="fchk" id="fold96"><label for="fold96">许多笔记软件引入了嵌套标签，或者说是父-子标签的功能。这相当于使用父标签对子标签做了进一步的分类，从而为所有标签赋予了层级式的分类结构，提高了多标签管理的效率。</label>
	...
	* <input type="checkbox" name="fchk" id="fold100"><label for="fold100">不过，尽管标签对具体笔记实现了多分类，但父标签对子标签所做的这种分类却往往只是单分类。文件夹单分类局限性所带来的问题，此时在另一个层次下出现了。</label>
		...
		* <input type="checkbox" name="fchk" id="fold104"><label for="fold104">此时也可以考虑不在软件中体现这些标签的父子关系，而是手动把有 `#李白诗` 标签的笔记也同时打上 `#文学/中国文学`、`#文学/古代文学` 这样的标签。不过这样的做法也有其局限性。</label>
	...
	* <input type="checkbox" name="fchk" id="fold110"><label for="fold110">用标签来查找特定笔记的时候，有观点认为其易用性可能不如文件夹和 MOC。这是因为比起逻辑关系，人脑更擅长记忆空间位置关系，而依据标签的查找本身不涉及空间感知，主要是依据笔记的逻辑关系进行的。</label>
	...
	* <input type="checkbox" name="fchk" id="fold116"><label for="fold116">标签可以体现笔记的属性，标明一条笔记从属于哪些集体关联。不过标签本身所能给出的信息常常不够充分：有的笔记只有几个句子与标签有关，而标签却为整条笔记标注了关联；有时还需要额外的解释性文字才能说明这个标签为什么成立，这单从标签本身是不容易看出来的。</label>
		* <input type="checkbox" name="fchk" id="fold117"><label for="fold117">Andy Matuschak《标签是无效的关联结构》表达了类似的观点。[英文原文](https://notes.andymatuschak.org/z3MzhvmesiD2htMaEFQJif7gJgyaHAQvKH49Z)，[中文翻译](https://zhuanlan.zhihu.com/p/425400912)</label>
	...
	* <input type="checkbox" name="fchk" id="fold126"><label for="fold126">作为一种集体关联方式，标签也是有缺陷的。和文件夹一样，一条标签下的所有笔记以简单并列的方式组织，通常没法指定特定的排列顺序，也难以为不同笔记添加更多的说明。</label>
		* 同一标签下的某些笔记其实相互之间的联系要更紧密一些；除此以外，不同笔记可能出于不同的原因打上了同一个标签，尽管它们都放在同一个标签之下，其地位也不见得是等同的。简单的标签并不能体现这些方面。
		* <input type="checkbox" name="fchk" id="fold128"><label for="fold128">若只希望在标签搜索结果中看到一些文字说明，一种可能的解决方案是用软件的全文搜索功能替代标签搜索。不过这么做仍有其局限性。</label>
		...
		* <input type="checkbox" name="fchk" id="fold136"><label for="fold136">有些软件支持保存标签的搜索结果，此时可以手动编辑这一结果，使它按自己想要的顺序排列，并加上特定的说明文字。</label>
	...
	* <input type="checkbox" name="fchk" id="fold141"><label for="fold141">随着内容增长，某标签下的笔记数量可能变得过多。此时需要对这一标签进行细化，分裂出若干个子标签。而这一操作可能也不是那么顺手。</label>
		...
		* <input type="checkbox" name="fchk" id="fold146"><label for="fold146">相比之下，如果是要对文件夹下的内容做进一步细分，只需要将笔记拖进新建的若干子文件夹之中，并不需要打开各个笔记文件，显得方便许多。</label>
			* 不过「不需要打开」只是针对细分的执行阶段而言的。对于执行前的判断阶段，如果某些笔记无法根据标题来判断其细分分类，则可能还是需要打开笔记查看其具体内容再决定。
		* <input type="checkbox" name="fchk" id="fold148"><label for="fold148">而如果是对一个 MOC 进行细分，也只是编辑一个或少数几个 MOC 文件，不需要打开各个笔记，同样十分方便。</label>
		...
		* <input type="checkbox" name="fchk" id="fold153"><label for="fold153">此外，相较文件夹和 MOC，使用标签时未必能及时地发现其下的内容过多。在意识到某个标签需要进行细分时，其下已有的内容可能已经增长到太多，再要细分就比较麻烦了。</label>
...
* <input type="checkbox" name="fchk" id="fold159"><label for="fold159">**空间邻近**：使用电子笔记时，笔记间空间邻近关系的作用容易被忽略，不过它仍是一种有价值的关联。</label>
	* 若将多条笔记排列在特定的空间位置上，则紧挨着的笔记之间形成了较强的成对关联，而相隔了一些距离的笔记之间也形成了弱关联。如此建立的空间邻近关系可认为是介于成对关联与集体关联之间的一种关联方式。
	* <input type="checkbox" name="fchk" id="fold161"><label for="fold161">要为笔记安排这样的特定空间位置，使用顺序结构是较为常见的做法。</label>
	...
	* <input type="checkbox" name="fchk" id="fold165"><label for="fold165">在一些大纲型双链笔记软件中，可以将许多（甚至是所有的）笔记放在同一个文件内，其中相关性高的笔记可以安排在同一个无序列表中的相邻位置。这为这些笔记建立起了空间邻近式的关联。</label>
		* <input type="checkbox" name="fchk" id="fold166"><label for="fold166">例如，deerain 所写的[《MOC - 管理链接而非本体》](https://www.yuque.com/deerain/gannbs/hb0gsd) 开头提到 Emacs org-mode 就可以把所有笔记都写到同一个文件里（需要结合一些特殊配置）。</label>
	...
	* <input type="checkbox" name="fchk" id="fold169"><label for="fold169">在某种意义上，卢曼所用的笔记序列可以看成大纲笔记在纸质时代的物理实现。</label>
		...
		* <input type="checkbox" name="fchk" id="fold175"><label for="fold175">另外，在大纲笔记的实际使用中，浅层级的点可能只是一个短语，而非一句或几句完整的话。卢曼二代卡片盒中编号较短的卡片也有着类似的特点，例如 [编号 9 的卡片](https://niklas-luhmann-archiv.de/bestand/zettelkasten/zettel/ZK_2_NB_9_V) 上就只写了「临时说明」一词。</label>
			* 这样看来，如果要实现电子版本的卢曼卡片盒，将每张卡片作为一个单独文件的做法多少显得有些奇怪。
		* <input type="checkbox" name="fchk" id="fold177"><label for="fold177">可以作出这样的猜想：如果一个大纲笔记用户某天突然回到了纸质时代，则无奈之下所形成的笔记记录方式很可能与卢曼的笔记序列类似。</label>
	...
	* <input type="checkbox" name="fchk" id="fold182"><label for="fold182">MOC 是建立空间邻近式关联的最灵活、最有效的方式。</label>
	...
	* <input type="checkbox" name="fchk" id="fold188"><label for="fold188">空间邻近有几个方面的作用。用笔记系统来辅助思考时，可以使用顺序结构表示想法的发展过程，此时相应的空间邻近关系展现了笔记的上下文。</label>
		* <input type="checkbox" name="fchk" id="fold189"><label for="fold189">Daniel [You Underestimate the Power of the Dark Folgezettel](https://strengejacke.wordpress.com/2015/11/01/you-underestimate-the-power-of-the-dark-folgezettel/) 一文认为两条笔记间的直接链接较为随意，其上下文不完全清楚，这与使用笔记序列继续一个想法不同。</label>
		...
		* <input type="checkbox" name="fchk" id="fold193"><label for="fold193">Eva Thomas [Understanding Zettelkasten — What does it mean to communicate with the slip-box](https://medium.com/@ethomasv/understanding-zettelkasten-d0ca5bb1f80e)（需翻墙）认为使用顺序结构容易组装出连贯的论点组合。此外，可以再引入单独的笔记来概括一串笔记序列的内容。</label>
	...
	* <input type="checkbox" name="fchk" id="fold203"><label for="fold203">空间邻近是需要主动设置的关联。在添加新笔记时，需要决定它适合放在哪些已有笔记附近，而这促使我们思考新笔记与已有笔记间的关系，从而加深对相应主题的理解，并可能发现新的联系。</label>
	...
	* <input type="checkbox" name="fchk" id="fold205"><label for="fold205">此外，将笔记放到相应位置的时候，往往还会重读附近的笔记，这可以（在非刻意复习的前提下）帮助强化对这些内容的记忆。</label>
		* yuchen_lea[《请不要神化双链笔记》](https://sspai.com/post/65273) 认为卢曼「将卡片放到对应位置的时候，会重读邻近位置、也就是相近主题下的其他卡片，强化对该主题的记忆。」
* <input type="checkbox" name="fchk" id="fold207"><label for="fold207">**链接**：最明确、精准的成对关联，也是卡片盒笔记法的要点之一。建立链接时通常需要附带一些文字说明，以使该关联更加具体明确。</label>
	* 链接在维基、学术引文网络等场合也十分常见。不过卡片盒笔记法中所用链接的含义与此并不完全相同，其中重要的一点是要与笔记原子化原则配合，从而允许建立更细粒度的、更精准的链接。
	* <input type="checkbox" name="fchk" id="fold209"><label for="fold209">有观点基于「思维的网状结构」而主张使用大量链接来组织笔记。这类笔记的阅读体验确实会与传统线性文章有所不同，可通过查看相应的公开笔记来获得一些直观感受，例如 Andy Matuschak 的系列文章。</label>
	...
	* <input type="checkbox" name="fchk" id="fold211"><label for="fold211">就卢曼本人而言，他将二代卡片盒作为自己在学术前沿探索、思考、创新的工具，并使链接有了更丰富的含义，其重要性甚至可以超过笔记的内容本身。这是在（通常意义上的）知识管理中几乎不可能出现的情况。</label>
	...
	* <input type="checkbox" name="fchk" id="fold221"><label for="fold221">建立链接时，附上一些文字描述通常会有帮助：这能帮助展现关联的背景，使其含义更丰富；简明扼要的文字也能在日后回顾笔记时提供引导。</label>
		* <input type="checkbox" name="fchk" id="fold222"><label for="fold222">Andy Matuschak 的笔记中有一篇是《优先使用经过标注的关联》（[中文翻译](https://zhuanlan.zhihu.com/p/425763923)，[英文原文](https://notes.andymatuschak.org/z7pGUpz2fQsHHUPbjThz85xXPvHwrmikAeYH4)），其中强调了对链接进行标注的重要性。</label>
...
* <input type="checkbox" name="fchk" id="fold229"><label for="fold229">**反向链接**：双链笔记软件的重要贡献。它通常是与链接配合的另一面，属于成对关联，而在某些情况下也能用来构建集体关联。</label>
	* 反向链接的展示利用了双链笔记软件提供的反向搜索功能。我们在笔记 A 中建立指向笔记 B 的链接之后，可以立刻在笔记 B 的下方看到笔记 A 引用了它，而无需进行额外的操作。
	* <input type="checkbox" name="fchk" id="fold231"><label for="fold231">如果只考虑追溯引用的目的，这实际上在非双链笔记中也能实现，只需要每次建立 A 到 B 的链接时，再同时在 B 中建立指向 A 的链接即可。双链笔记软件则给了用户省下这一步操作的可能性。</label>
		* 卢曼在[《与卡片盒交流》](https://mp.weixin.qq.com/s/_aPS2ol1DxiCFX-xfl-8yg)一文中就提到了这样的做法，在建立链接时，「也要马上在被链接到的笔记中记录下反向链接。」
	* <input type="checkbox" name="fchk" id="fold233"><label for="fold233">不过同样作为成对关联方式，反向链接仍与直接链接不同。直接链接是主动建立的关联，其建立可以有选择性，也能补充相应的文字描述；而被动建立的反向链接不具有这些特点，其实用性比起直接链接要相对低一些。</label>
	...
	* <input type="checkbox" name="fchk" id="fold239"><label for="fold239">反向链接还为笔记软件提供了一种新的用法。我们可以建立指向一条空笔记的链接，在这样的链接较多之后，从这条空笔记的反链列表里就可以看到许多与之相关的内容。这可认为是依靠反链建立了某种集体关联。</label>
		* <input type="checkbox" name="fchk" id="fold240"><label for="fold240">Roam Research、Logseq 等笔记软件中的标签是正文中形如 `#标签` 的文本，而这些软件实际上将标签当成了一种特殊的链接。我们要搜索这个标签下的内容时，其实是点进这个标签所对应的（空）笔记，并查看它的反链列表。也就是说，其中的标签搜索功能实际上是通过反链搜索来实现的。</label>
	...
	* <input type="checkbox" name="fchk" id="fold244"><label for="fold244">此外，反链在每日笔记（daily notes）工作流中也可发挥其作用。这允许我们将一些临时产生的信息先暂时存放在每日笔记中，而只在将来需要的时候才将之转移进相应主题的笔记。</label>
...
* <input type="checkbox" name="fchk" id="fold249"><label for="fold249">**MOC（Map of Contents，内容地图）**：基于链接建立起的关联，也是最强大、最灵活的集体关联方式。MOC 的引入也使笔记系统具有了更丰富的结构。</label>
	* 注：这个概念其实有非常多的同义词，博客里使用「MOC」这个称呼主要是因为它的知名度比较高。关于这个称呼的更多讨论可以在 [MOC的组织](../MOC的组织/) 一文的开头看到。
	* 如果我们找出许多相同类别/相同主题的笔记，并将指向它们的链接都汇总到一条新的笔记里，那么这条新笔记就成为了一个 MOC。
	* 而另一方面，由于在这里能同时看到许多的同类别、同主题的笔记，MOC 也就以这种方式为它们建立起了集体关联。
	* <input type="checkbox" name="fchk" id="fold253"><label for="fold253">MOC 的引入使笔记系统中链接的类型变得更为多样。原来只有普通笔记之间的链接，而现在多了 MOC 指向普通笔记的链接，此外还可以有普通笔记指向 MOC 的链接。</label>
	...
	* <input type="checkbox" name="fchk" id="fold257"><label for="fold257">此外，如果一个 MOC 中引用的笔记数量变得比较多了，我们还可以把它拆成几个小的 MOC，然后在一条新笔记中存放指向这些小 MOC 的链接。</label>
	...
	* <input type="checkbox" name="fchk" id="fold260"><label for="fold260">在用于进行主题汇总时，MOC 将显得更为强大。它使我们能够更有效地管理这一主题，加深对其中内容的理解。</label>
		...
		* <input type="checkbox" name="fchk" id="fold264"><label for="fold264">在进一步引入指向 MOC 的「高阶 MOC」之后，其处理多主题的潜能将得到进一步的释放。</label>
	...
	* 随着 MOC 下引用的笔记逐渐增多，其内涵变得更为丰富，而相应的内容组织方式或许也需要做出调整。[MOC的组织](../MOC的组织/) 一文对此进行了更为详细的讨论。
	* <input type="checkbox" name="fchk" id="fold276"><label for="fold276">在有了 MOC 之后，向笔记系统添加新笔记的工作流也有了更多内容，这包括将它加入相关的 MOC，以及建立一个新 MOC 等。</label>
