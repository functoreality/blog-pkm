...
* 关联可以大致分为成对关联和集体关联两种。前者如链接，一次只关联起两条笔记；后者如文件夹和标签，可以将许多笔记关联起来。分类和主题是建立集体关联的常用方式。
...
* **文件夹**：若要建立分类式的集体关联，使用文件夹是最简单、直观的方式，不过它不适合处理一条笔记有多个分类的情况。文件夹的功能理论上可被 MOC 替代。
	...
	* 文件夹分类管理方式的有效性相当程度上依赖于 MECE 原则，而实践中频繁出现的多分类情况往往造成严重的干扰。新加入的笔记可能同时属于多个类别；此外分类标准可能并不唯一，采用不同的标准可以给出不同的分类结果。
		...
		* 为了应对这种情况，可以考虑使用系统的快捷方式、使用标签或者 MOC 来作为文件夹的替代品。
			* 系统快捷方式允许从不同的文件夹下访问同一个文件。不过这不便于备份以及多设备间同步，在笔记文件位置改变（比如重新分类、挪入新建的子文件夹）时会失效，比较受限。
			* 一些笔记软件可以体现标签之间的层级关系，也就是所谓的「嵌套标签」。还用文学作品的例子，此时关于某个作品的一条记录可以同时有 `#文学作品/按地区分类/英美`，`#文学作品/按体裁分类/诗歌/十四行诗`这两个标签。
	...
	* 考虑到个人认知有一个发展的过程，我们完全有可能在未来的某一天发现，根据自己之前的粗浅理解所设立的分类方式已经不再适用了。对文件夹管理方式来说，这种重新分类的过程可能是相当痛苦的。
		* 使用标签进行的重分类则要相对轻松一些。使用标签的分类体系不像文件夹那样有排他性，这允许我们在不破坏旧的分类体系的情况下，为笔记补上新分类标准下的对应标签。
		...
		* 使用 MOC 进行的重分类同样十分灵活，我们只需要建立一个新的 MOC（或在原有 MOC 的前面或后面开辟一段新的空间），用来存放新标准下的分类结果就可以了。
			...
			* 即使在新标准的 MOC 建立完成以后，我们也没必要删除原来旧标准下的 MOC。只需要在旧版本中做适当的标注，并给出指向新版本 MOC 的链接即可。
	...
	* 此外，文件夹下的内容以简单并列的方式呈现，但它们之中其实有一些存在更紧密的联系（其数量又不值得单独建立一个子文件夹），或者其关系值得进行特殊的说明。这类具体而精细的关联难以使用「同分类」的简单方式来表达。
		* 若设定文件名以使一些文件显示在相邻位置，可以部分地表达前一种关联。
...
* **反向链接**：双链笔记软件的重要贡献。它通常是与链接配合的另一面，属于成对关联，而在某些情况下也能用来构建集体关联。
	...
	* 反向链接还为笔记软件提供了一种新的用法。我们可以建立指向一条空笔记的链接，在这样的链接较多之后，从这条空笔记的反链列表里就可以看到许多与之相关的内容。这可认为是依靠反链建立了某种集体关联。
		* Roam Research、Logseq 等笔记软件中的标签是正文中形如 `#标签` 的文本，而这些软件实际上将标签当成了一种特殊的链接。我们要搜索这个标签下的内容时，其实是点进这个标签所对应的（空）笔记，并查看它的反链列表。也就是说，其中的标签搜索功能实际上是通过反链搜索来实现的。
