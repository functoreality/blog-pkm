* 首先声明，这篇文章中将要介绍的笔记方法主要面向理论文章阅读、思考工具的需求，参见 [[笔记的不同目的]]。对于有其他需求（比如狭义的知识管理）的读者，我不保证这里的方法对你有参考价值。
...
* 不过我并没有沿用「卡片盒笔记法」的称呼，而是采用了一个稍不一样的名字。这当中有多个方面的考量。
	...
	* 第二，本文所理解的卡片盒方法与当前互联网上绝大多数讨论中的确实有很大不同。为避免可能的误解，我使用了一个新名称，这并不代表我认为这是自己独创的方法。
		...
		* 而 Ahrens 所描述的方法与卢曼的方法有许多关键的不同，我在 [[笔记的不同目的]] 中已经对此进行了部分讨论。
...
* 而卢曼的一张卡片实际上不能用「自成一体」来描述，也即它尽管是笔记管理（添加、链接）中的原子化单位，但却并非传统意义上的「一条笔记」，无法独立于特定的上下文语境而存在。
	* 卢曼在《与卡片盒交流》中提到：「原本连续的文本经常被数百张插入的新卡片打断；但如果系统地处理编号，就很容易找回原有文本的全貌。」
		...
		* Schmidt 也对此进行了确认，他在卢曼的卡片盒中多次发现了上百张的卡片，它们「后来被插入到最初在相关主题上同时创建的、两张连续的卡片之间」。
	...
	* 关于这种拆分、从中插入新卡片做法的意义，读者可以直接看看这篇文章的组织方式，文中有许多内容是默认折叠的。如果这些折叠部分的内容可以在日后随时添加，它所带来的好处相信读者自己就能够理解。
...
* 如果进一步考虑大纲笔记，那么大纲所代表的树形组织结构正好成了卢曼式卡片编号的完美替代品，我们连编号的过程都免去了。
	...
	* 大纲结构与卢曼卡片编号的具体对应方式其实不难理解，很多人都自己意识到了这一点。如果有读者希望了解这一对应关系的具体细节，可以看看 [[笔记间关联方式]]，我在讨论空间邻近关系的时候，就解释了「卢曼所用的笔记序列可以看成大纲笔记在纸质时代的物理实现」。
...
* 进一步，由于这种树形结构仅在组织上、而不在内容上成立，它对我们查找特定的笔记不总是有帮助。卢曼据此制作了「索引笔记」，作为他的笔记系统的入口。
	...
	* 在这一方面，[[MOC的组织]] 这篇文章仍有一定的参考价值（尽管作为我旧有认知的见证，其中不正确地假定了各条笔记是自成一体的）。我们可以让索引中的关键词条目指向一些（可能位于笔记深处的）MOC，让它们充当我们查找笔记的跳板。
	...
	* 另外，特定关键词所指向的笔记有时也并非对这一关键词本身的解释。
		...
		* 我也在自己的笔记系统中尝试了类似的做法。例如，我的索引笔记中「公众人物」关键词所指向的笔记就在探讨「网络科学中枢纽节点效应在社会网络中的可能表现」，其中甚至根本没有出现「公众人物」这个词。
...
* 在我们阅读文章、制作了文献笔记以后，我们可以通过「跨语境翻译」的方式，将其中有价值的内容全部整合进由主笔记组成的系统当中，从而彻底取消不同文章、不同主题、不同学科之间的边界。
	...
	* 由于这种整合的依据是语境，而不是某篇文章（像原始的文献笔记那样），也不是某个特定的主题（像笔记中的多数 MOC 一样），笔记变成了几乎没有边界的「开放系统」，在这一方面具有超越传统笔记方法的强大能力。
