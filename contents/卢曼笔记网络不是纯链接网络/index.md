...
* <input type="checkbox" name="fchk" id="fold51"><label for="fold51">网络科学中有「小世界网络」的概念，而这种组合关联的引入有希望使笔记网络获得这样的「小世界」性质，笔记中增强的关联能力与发散性也可以视为这一性质的自然结果。</label>
	* <input type="checkbox" name="fchk" id="fold52"><label for="fold52">如果把每条笔记作为网络中的一个顶点，而笔记间的关联视作网络中的连接，那么笔记系统在这个意义下具有了「网状结构」，可以从网络科学的视角进行解读。</label>
		* 注意这里使用网络分析笔记系统的方式与常见的不同。常见的方式仅仅将笔记中的「链接」作为网状结构中的「连接」，许多双链笔记软件中自动生成的「笔记关系图谱」就是基于这种理解方式进行了网络结构的可视化。
		...
		* 从笔记系统使用方式的角度来看，这种理解方式也具有合理性。如果要讨论给定网络模型上的游走过程，从一个顶点到另一个顶点的移动通常只能沿着网络中的「连接」进行。而在笔记系统中，我们可以方便地从一条笔记看到另一条与之形成了关联的笔记，无论这种「关联」是由链接还是笔记序列给出的。
	...
	* <input type="checkbox" name="fchk" id="fold58"><label for="fold58">在常见的网络模型中，一类特殊的网络被称为「小世界网络」。粗略地来说，小世界网络大致是这样的一类网络，它有许多的顶点、相对稀疏的连接，而任意两个顶点在网络中都相距不远。</label>
		* 这里所说的「相距不远」指的是：从一个顶点出发，沿着网络中的连接行进，我们只需要经过为数不多的中间顶点，即可到达另一个顶点。
		* <input type="checkbox" name="fchk" id="fold60"><label for="fold60">「小世界网络」最广为人知的例子可能是社会网络中的「六度分隔」理论。这一理论提出，一个人直接认识的人通常不多，而认识的人所认识的人则多不少，他们再认识的人就要更多了。当这种「认识」嵌套到六层时，通常已足以覆盖当今世界上的大部分人。</label>
			*  如果将人视为社会网络中的顶点，人之间相互认识的关系作为网络中的连接，则「六度分隔」理论便意味着，这样一个社会网络是一个「小世界网络」。
				<!-- TODO：参考网上已有表述 -->
	* <input type="checkbox" name="fchk" id="fold63"><label for="fold63">网络科学家已经提出了许多「小世界网络」的模型，其中的很多模型涉及两种类型的连接：一种是局部的、密集的连接，另一种是远程（全局）的、相对稀疏的连接。</label>
		*  以 Newman-Watts 网络为例。在这个网络模型中，所有顶点排列在一个圆上，圆上的相邻顶点建立连接，也可以额外再为相距不太远（比如距离不超过 2）的顶点对建立连接。这是网络中的第一种连接。
			<!-- * 这里为相距不远、但非直接相邻的顶点对建立连接 -->
		* 在这一基础上，网络中的每个顶点以一个相对较小的概率发出一条新的连接，连接所涉及的另一顶点从整个网络的所有顶点中随机选择。这是网络中的第二种连接。
		*  下面这张图片展示了一个 Newman-Watts 网络的例子（来自论文：TODO）
			<!-- TODO：img -->
	* 而在笔记系统中，笔记序列建立了局部的集体关联，相当于这里的第一种连接；直接链接则是跨区域的成对关联，相当于这里的第二种连接。由此看来，笔记通过这两种关联形成一个「小世界网络」的可能性是不容忽视的。
	* 这样的小世界性质拉近了不同笔记之间的距离。从某条笔记出发，即使只进行有限次的跳转，可访问到的笔记的范围也是巨大的。笔记网络由此为我们提供了十分丰富的可能性。
* （施工中）
<!-- 有 TODO 待解决，需全文搜索 -->
